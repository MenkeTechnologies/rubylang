...
//!
//! `ruby --tiers script.rb` prints the report.
...
pub struct ChunkTiers {
    /// Which chunk this is — `main` for a whole Ruby program.
    pub name: String,
...
/// has gone round enough times to be recorded. rubylang writes program output
...
///
/// A Ruby program is not one chunk: the top level, every `def`, every method of
/// every class, and every block/proc body lower to their own chunk. A hot loop
...
fn program_chunks(prog: &crate::compiler::Program) -> Vec<(String, Chunk)> {
    ...
    // already covered by the proc list rather than needing their own walk.
    for (i, p) in prog.procs.iter().enumerate() {
        out.push((format!("block #{i}"), p.chunk.clone()));
    }
    out
}
